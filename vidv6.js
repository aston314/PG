    ...
    if (path.startsWith("/vidsrc/")) {
        ...
        try {
            ...
            if (VidlinkProData && upcloudData && VidlinkProData.data.subtitles.length == 0) {
                // Merge subtitles from upcloud into Vidsrc.net data
                const mergedSubtitles_ = [
                    ...(VidlinkProData.data.subtitles || []),
                    ...(upcloudData.data.subtitles || [])
                ];

                // Remove duplicates based on label
                const uniqueSubtitles_ = mergedSubtitles_.reduce((acc, current) => {
                    const x = acc.find(item => item.label === current.label);
                    if (!x) {
                        return acc.concat([current]);
                    } else {
                ...
                // Update Vidsrc.net data with merged subtitles
                VidlinkProData.data.subtitles = uniqueSubtitles_;
            }
